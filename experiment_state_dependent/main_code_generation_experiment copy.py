...
def DiTree_to_newick_string_bifur(subtree, node, correction, root):
    childeren = list(subtree.neighbors(node))
    if len(childeren) == 0:
        return str(node)+f":{1-correction}"
    elif len(childeren) == 1:
        if node == root:
            return "(" + str(DiTree_to_newick_string_bifur(subtree, childeren[0], correction, root)) + f"):{1-correction}"
        else:
            return str(DiTree_to_newick_string_bifur(subtree, childeren[0], correction - 1, root))
    elif len(childeren) == 2:
        return "(" + str(DiTree_to_newick_string_bifur(subtree, childeren[0], 0, root)) + "," + str(DiTree_to_newick_string_bifur(subtree, childeren[1], 0, root)) + f"):{1-correction}"
    else:
        random.shuffle(childeren)
        corrections = [random.random()/100 for _ in range(len(childeren) - 2)]
        string = "(" + DiTree_to_newick_string_bifur(subtree, childeren.pop(), sum(corrections), root) + "," + DiTree_to_newick_string_bifur(subtree, childeren.pop(), sum(corrections), root) + f"):{corrections[-1]}"
        for i, child in enumerate(childeren[:-1]):
            string = "(" + DiTree_to_newick_string_bifur(subtree, child, sum(corrections[:-(1+i)]), root) + "," + string + f"):{corrections[-(2+i)]}"
        return "(" + DiTree_to_newick_string_bifur(subtree, childeren[-1], 0, root) + "," + string + f"):{1-correction}"

def write_to_nexus(newick_string, ntax, df, seed):
    with open(f"experiment_real/real_tree_exp_{seed}.nex", "w") as file:
...
def run_and_write_nexus(df, country, tree, names, seed, fasta = False, nexus = False):
    if fasta:
    ...
    if nexus:
        ...
        newick_string = DiTree_to_newick_string_bifur(subtree_relabled, ntax + 1, correction = 0, root = root)
        # up to this can go before the loop
        write_to_nexus(newick_string, ntax, df, seed)
...
def main(graph_name = "complete", n = 500, distribution = None, contact_matrix = None, beta = 0.5, num_seeds = 100, sampling = 1, gamma = 1, fasta=False, nexus=False, heuristic=False, heuristic_informed=False):
    # to do:
    ...
    if distribution == None:
        distribution = [n]
        transition_matrix = beta*np.array([[5]])/n
        seed, seeds = 0, []
        ...
        transition_matrix = beta*contact_matrix/n
        seed, seeds = 0, []
    
    while len(seeds) < num_seeds:
        seed += 1
        country, tree, names, df, _ = run_one_epi(graph_name, beta, n, seed, distribution, transition_matrix, gamma = gamma)

        df_sample = df[df["date"] == df["date"].mode().max()]
        if len(df_sample) > 1: 
            seeds += seed,
            run_and_write_nexus(df_sample, country, tree, names, seed, fasta=fasta, nexus=nexus)
...
if __name__ == "__main__":
    distribution = [250, 250]
    contact_matrix = np.array([[10, 1], [1, 10]])
    main(n = 500, distribution = distribution, contact_matrix = contact_matrix, num_seeds=100, nexus = True)
# %%
