...
for exp in exps:
    ...
    if heuristic:
        df_sample_hear = pd.read_csv(f"experiment_heuristic/df_childeren_exp_{exp}.csv")
        np.random.seed(exp)
        parents_index_heur, max_lik = estimate_parents(df_sample_hear, params_df["gamma"].item())
        if exp == exps[0] and filter_liks:
            # hardcode
            max_lik_set = set(max_lik[1:])
            max_lik_list = list(sorted(max_lik_set))
    ...
    for proba in probas:
        ...
        if heuristic and filter_liks:
            if max_lik_set.issubset(set(max_lik[1:])):
                proba_infer_include += 1,
                for lik in max_lik_list:
                    indexes = np.argwhere(max_lik >= lik).flatten()[1:]
                    parent_types_heur = np.array([childeren_types[i] for i in parents_index_heur])
                    proba_infer_heur += estimate_jump_proba(parent_types_heur[indexes], childeren_types[indexes]),
            else:
                proba_infer_include += 0,
                proba_infer_heur += len(max_lik_list)*[0]
        elif heuristic:
                parent_types_heur = np.array([childeren_types[i] for i in parents_index_heur])
...
if filter_liks:
    exp_df = list(itertools.chain.from_iterable(len(probas)*len(max_lik_list)*[exp] for exp in exps))
    proba_df = list(itertools.chain.from_iterable(len(max_lik_list)*[proba/100] for _ in exps for proba in probas))
else:
...
if heuristic:
    ...
    if filter_liks:
        df_output["include_lik"] = list(itertools.chain.from_iterable(len(max_lik_list)*[incl] for incl in proba_infer_include))
        with open("likelihoods.txt", "w") as file:
            for lik in max_lik_list:
                file.write(str(lik)+ "\n")
